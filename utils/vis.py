...
def get_label_name_colors(csv_path):
    ...
    """
    label_names, label_colors = ['bg'], [[0, 0, 0]]
    ...
    return label_names, np.array(label_colors, dtype='uint8')
...
def plt_img_target_pred(img, target, pred, label_colors,
                        dpi=100,
                        vertial=False,
                        save_path=None, title=None, vis=True):
    ...
    if vertial:
        f, axs = plt.subplots(nrows=3, ncols=1, dpi=dpi)
        f.set_size_inches((4, 9))
    else:
        f, axs = plt.subplots(nrows=1, ncols=3, dpi=dpi)
    ...
    if title:
        plt.title(title)  # acc 放在 pred 图上
...
def plt_img_target_pred_error(img, target, pred, error_mask, label_colors,
                              save_path=None,
                              dpi=100,
                              title=None, vis=True):
    f, axs = plt.subplots(nrows=1, ncols=4, dpi=dpi)
