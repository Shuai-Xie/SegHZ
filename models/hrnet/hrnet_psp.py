...
from modules.attention.PSP import PPM
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, with_aux=False, **kwargs):
        """
        ...
        ## PSP module lightweigt 960 + 120*4
        reduction_dim = last_inp_channels // 8
        self.ppm = PPM(last_inp_channels, reduction_dim, bins=(1, 2, 3, 6))  # 128; 512/32=16
        self.cls_head = nn.Conv2d(in_channels=last_inp_channels + reduction_dim * 4,
    ...
    def forward(self, x):
        ...
        ## PSP
        ppm_feats = self.ppm(feats)
        out = self.cls_head(ppm_feats)  # 1/4
...
def HRNet_PSP(cfg_path, num_classes, use_pretrain=True, with_aux=False, **kwargs):
    from models.hrnet.config import update_config

    cfg = update_config(cfg_path, num_classes)
    model = HighResolutionNet(cfg, with_aux=with_aux, **kwargs)
    if use_pretrain:
...
if __name__ == '__main__':
    ...
    model = HRNet_PSP(f'models/hrnet/cfg/seg_hrnet_w64.yaml', 7, use_pretrain=False)
