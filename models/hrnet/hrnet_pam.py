...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        # channel sum 48,96,192,384 = 720 w48/ w64
        ...
        self.pam = nn.Sequential(
            # 爆内存 downsample 1/8 64*64
            conv3x3(last_inp_channels, last_inp_channels, stride=2),
            BatchNorm2d(last_inp_channels),
            nn.ReLU(inplace=True),
            # pam
            PAM(in_dim=last_inp_channels, reduction=8),
            # upsample x2, and reduce dim
            nn.ConvTranspose2d(last_inp_channels, last_inp_channels // 2, kernel_size=4, stride=2, padding=1, bias=False),  # k= 2p+s
            BatchNorm2d(last_inp_channels // 2),
            nn.ReLU(inplace=True),
        )
        self.cls_head = nn.Conv2d(in_channels=last_inp_channels + last_inp_channels // 2,
    ...
    def forward(self, x):
        ...
        feats = torch.cat([x[0], x1, x2, x3], 1)  # 960
        out_aux = self.aux_head(feats)  # 原始分割结果, b,19,h,w

        ## PAM
        att_features = self.pam(feats)
        feats = torch.cat((feats, att_features), dim=1)  # 原始 feature 学到好基础上，concat att feature
        out = self.cls_head(feats)  # 1/4
...
def HRNet_PAM(cfg_path, num_classes, use_pretrain=True, **kwargs):
...
if __name__ == '__main__':
    model = HRNet_PAM(f'models/hrnet/cfg/seg_hrnet_w64.yaml', 7, use_pretrain=False)
